...
Handler.prototype.follow = function(msg, session, next) {
  ...
  async.waterfall([
    ...
    function(_uid, _cb) {
      // 查询当前关注状态
      self.app.rpc.user.userRemote.getFollowState(session, followid, _uid, function(_err, _isSure) {
        if (!!_err) {
          _cb(_err);
        } else if (_isSure) {
          _cb('已经关注过了');
    ...
    function(_uid, _cb) {
      // 添加关注绑定
      self.app.rpc.user.userRemote.addFollower(session, followid, _uid, function(_err) {
        _cb(_err);
      });
    },
    function(_cb) {
...
Handler.prototype.unfollow = function(msg, session, next) {
  ...
  async.waterfall([
    ...
    function(_uid, _cb) {
      // 删除关注绑定的关系
      self.app.rpc.user.userRemote.deleteFollower(session, followid, _uid, function(_err) {
        _cb(_err);
      });
    },
    function(_cb) {
